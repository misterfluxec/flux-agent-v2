...
const TABS = [
  { id: "learn", label: "Fuentes", icon: Upload, desc: "Agrega documentos o URLs" },
  { id: "process", label: "Estado", icon: Cpu, desc: "Monitoreo de ingesta" },
  { id: "assign", label: "Agentes", icon: Link2, desc: "Asigna contexto a IAs" },
];
    ...
    <div className="space-y-6 animate-in fade-in duration-700 pb-12 px-4 md:px-8 max-w-7xl mx-auto pt-8">
      ...
      <div>
        <h1 className="text-3xl font-black tracking-tight text-white/90">
          <span className="text-cyan-400">Base de Conocimiento</span>
        </h1>
        <p className="text-white/40 text-sm mt-1">
          Documentos, políticas, manuales, FAQs y contexto semántico para tus agentes.
        </p>
      </div>
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h2 className="text-lg font-black text-white">Agregar Fuente</h2>
          <p className="text-xs text-white/30">Sube documentos PDF, manuales, FAQs o conecta URLs</p>
        </div>
      </div>
      ...
      <label className="block cursor-pointer">
        <div className="border-2 border-dashed border-white/10 hover:border-cyan-500/30 rounded-2xl p-10 text-center transition-all hover:bg-cyan-500/[0.02] group">
          ...
          <p className="text-xs text-white/20 mt-2">PDF, TXT, DOCX — Máximo 50MB</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { icon: FileText, label: "Documentos", desc: "PDFs, Manuales, Políticas, FAQs" },
          { icon: Globe, label: "Web / URL", desc: "Sitios, landing pages, blogs" },
        ].map(s => (
    ...
    <div className="p-6 md:p-8 space-y-5 animate-in fade-in duration-300">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-white">Estado de Conocimiento</h2>
            <p className="text-xs text-white/30">{totalChunks} elementos de contexto indexados</p>
          </div>
        </div>
          ...
          {sources.map(s => (
            <div key={s.fuente_nombre} className="flex items-center gap-4 px-5 py-4 bg-white/[0.02] border border-white/5 rounded-xl hover:border-white/10 transition-colors group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-white/30">{s.chunks} elementos · {s.fuente_tipo?.toUpperCase()} · {new Date(s.ultima_ingesta).toLocaleDateString()}</p>
              </div>
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] px-2 py-1 rounded-lg bg-cyan-500/10 text-cyan-400/70 font-bold">
                    {agent.knowledge_base_size || 0} elementos
                  </span>
              ...
              <div className="flex flex-wrap gap-2">
                {agent.rag_collection ? (
                  ...
                  <p className="text-[11px] text-white/20 italic">
                    Acceso al conocimiento global del tenant ({sources.reduce((s, f) => s + f.chunks, 0)} elementos disponibles)
                  </p>
                ) : (
