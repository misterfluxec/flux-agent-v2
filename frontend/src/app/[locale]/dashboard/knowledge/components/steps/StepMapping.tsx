...
export function StepMapping({ headers, mapping, onMappingChange, onConfirm, onBack, isReady }: any) {
  ...
  const OPTIONAL = ['sku', 'stock', 'description', 'categoria'];

  const autoDetect = (field: string) => {
    const lower = field.toLowerCase();
    if (['name', 'producto', 'item', 'name'].some(k => lower.includes(k))) return 'name';
    if (['price', 'costo', 'valor', 'price'].some(k => lower.includes(k))) return 'price';
    ...
    if (['desc', 'detalle', 'description'].some(k => lower.includes(k))) return 'description';
