...
export function WizardStepper({ currentStep, isProcessing }: { currentStep: number; isProcessing: boolean }) {
  return (
    <div className="flex items-center justify-between mb-10 relative px-4">
      {/* Background Line */}
      <div className="absolute top-1/2 left-4 right-4 h-[2px] bg-white/10 -z-10 transform -translate-y-1/2 rounded-full" />
      
      {/* Progress Line */}
      <div 
        className="absolute top-1/2 left-4 h-[2px] bg-gradient-to-r from-primary to-blue-500 -z-10 transform -translate-y-1/2 transition-all duration-700 ease-in-out shadow-[0_0_10px_rgba(6,182,212,0.5)]"
          ...
          <div key={step.id} className="flex flex-col items-center gap-3 bg-transparent relative">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center border-[3px] transition-all duration-500 backdrop-blur-md ${
              isCompleted 
                ...
                : isActive 
                  ? 'border-primary bg-primary/20 text-primary scale-110 shadow-[0_0_20px_rgba(6,182,212,0.4)]' 
            ...
            <span className={`absolute -bottom-6 text-[10px] font-bold uppercase tracking-wider whitespace-nowrap transition-colors duration-300 ${
              isActive ? 'text-primary' : isCompleted ? 'text-emerald-500' : 'text-white/30'
            }`}>
              {step.label}
            </span>
          </div>
