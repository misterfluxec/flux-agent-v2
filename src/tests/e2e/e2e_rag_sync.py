...
# Mock de datos
MOCK_SHEET_DATA = (
    ["name", "price", "stock", "sku", "categoria", "description"],
    [
        {"name": "Laptop Pro", "price": "1200", "stock": "15", "sku": "LP-001", "categoria": "Laptops", "description": "Potente laptop para devs"},
        {"name": "Mouse Inalámbrico", "price": "25", "stock": "50", "sku": "MS-002", "categoria": "Accesorios", "description": "Ergonómico y veloz"},
        {"name": "Monitor 4K", "price": "350", "stock": "10", "sku": "MN-003", "categoria": "Monitores", "description": "Colores precisos"}
    ]
)
...
async def test_e2e_rag_sync_flow():
    ...
    sql_setup = f"""
    INSERT INTO tenants (id, name, status) VALUES ('{tenant_id}', 'Tenant E2E', 'is_active');
    INSERT INTO agents (id, tenant_id, name, status, model) VALUES ('{agent_id}', '{tenant_id}', 'FluxAgent', 'active', 'nomic-embed-text');
    ...
    VALUES ('{source_id}', '{tenant_id}', '{account_id}', '{agent_id}', 'google_sheets', 'SHEET123', 'Inventario Test', '{{"name": "name", "price": "price"}}');
    """
    ...
    await conn.execute(f"INSERT INTO tenants (id, name, status) VALUES ('{tenant_id}', 'Tenant E2E', 'is_active')")
    await conn.execute(f"INSERT INTO agents (id, tenant_id, name, status, model) VALUES ('{agent_id}', '{tenant_id}', 'FluxAgent', 'active', 'nomic-embed-text')")
    ...
    await conn.execute(f"INSERT INTO synced_sources (id, tenant_id, account_id, agent_id, source_type, source_id, source_name, column_mapping) VALUES ('{source_id}', '{tenant_id}', '{account_id}', '{agent_id}', 'google_sheets', 'SHEET123', 'Inventario Test', '{{\"name\": \"name\", \"price\": \"price\"}}')")
    await conn.close()

    # Autenticación
    token = crear_token_acceso(data={"sub": "usuario_test", "tenant_id": tenant_id, "role": "admin"})
    ...
    with patch('services.spreadsheet_sync_rag.RAGSyncEngine.fetch_google_sheet_data', return_value=MOCK_SHEET_DATA), \
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            ...
            payload = {
                ...
                "column_mapping": {"name": "name", "price": "price", "stock": "stock"},
