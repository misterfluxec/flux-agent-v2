...
from api.routers.communications.chat_router import procesar_mensaje_entrante
...
async def get_channel_status(channel_name: str, tenant_id: str = Header(None)):
    """Obtiene el status en vivo de un canal para un tenant."""
    if not tenant_id:
        # Modo mock para el desarrollo frontend UI
        return {"status": "connected", "latency_ms": 45, "health_score": 100}
    ...
    try:
        ...
        async with sesion_db() as db:
            res = await db.execute(text("""
                SELECT status 
                FROM canales_config 
                WHERE canal = :canal AND tenant_id = :tid
                LIMIT 1
            """), {"canal": channel_name, "tid": tenant_id})
            config = res.fetchone()
            
        if config:
            return {"status": config.status, "health_score": 100, "latency_ms": 45}
        return {"status": "disconnected", "health_score": 0, "latency_ms": 0}
    except Exception as e:
        logger.error(f"Error status channel: {e}")
