...
async def enhance_agent_prompt(
    ...
    """
    Toma una idea en crudo del cliente y devuelve un System Prompt 
    profesional y optimizado utilizando un LLM local. Utiliza las reglas
    de las Skills 'prompt-engineering-patterns' y 'enhance-prompt'.
    """
    import httpx
    import os
    import logging
    ...
    meta_prompt = f"""
Eres un Arquitecto de IA Experto en Prompt Engineering.
Tu tarea es tomar la "Idea en Crudo" del usuario y transformarla en un SYSTEM PROMPT perfecto y definitivo que será usado por un Agente IA.

REGLAS DE DISEÑO AVANZADAS:
1. ROL Y MISIÓN: Define claramente quién es el agente y su propósito central.
2. TONO Y PERSONALIDAD: Establece el estilo de comunicación de forma explícita.
3. INSTRUCCIONES Y LÍMITES: Define reglas estrictas (Ej: No inventar información, no alucinar precios, pedir intervención humana si no sabe).
4. FLUJO DE TRABAJO: Proporciona pasos lógicos de cómo debe resolver la intención del usuario.
5. CONTEXTO DEL NEGOCIO: {request.business_context}

IDEA EN CRUDO DEL USUARIO:
"{request.raw_idea}"

Devuelve ÚNICAMENTE el System Prompt final optimizado, sin introducciones, sin comillas externas ni comentarios adicionales.
"""

    try:
        # Intentamos usar Ollama local (modelo base)
        ollama_url = os.getenv("OLLAMA_COPILOT_URL", "http://172.21.0.1:11434")
        async with httpx.AsyncClient(timeout=45.0) as client:
            resp = await client.post(
                f"{ollama_url}/api/generate",
                json={
                    "model": "qwen2.5:3b",  # El modelo local default
                    "prompt": meta_prompt,
                    "stream": False,
                    "options": {
                        "temperature": 0.2
        ...
        logging.error(f"Error optimizando prompt: {e}")
        return {
            "status": "error",
            "message": str(e),
            "enhanced_prompt": request.raw_idea  # Fallback a la idea original
        }
